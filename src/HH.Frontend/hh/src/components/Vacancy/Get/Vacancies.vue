<template>
    <section class="bg-gray-50 dark:bg-gray-900 min-h-screen">
      <div class="container mx-auto py-8">
        ...
        <!-- Загрузка данных -->
        <div v-if="loading && vacancies.length === 0" class="text-center text-gray-500 dark:text-gray-400">Загрузка...</div>
        
        <!-- Если вакансий нет -->
        <div v-if="vacancies.length === 0 && !loading" class="text-center text-gray-500 dark:text-gray-400">Нет доступных вакансий</div>
  ...
  export default {
    ...
    setup() {
      ...
      const fetchVacancies = async () => {
        ...
        try {
          const response = await axios.get(`https://localhost:64917/api/Vacancy/getAll?page=${page.value}&pageSize=${pageSize.value}`);
